...
//
// IMPORTANT — directive order. Caddy's Caddyfile adapter reorders
// top-level directives by a hard-coded schema in which `file_server`
// runs before any third-party directive, so a naïve template like
// `forward_proxy ; file_server` actually compiles to a route where
// file_server responds first (200 to /, 404 elsewhere) and
// forward_proxy never runs — CONNECT is then served by Caddy's
// generic HTTP machinery, returning a no-op 200 with empty body.
// Wrapping forward_proxy + root + file_server in a `route { }` block
// preserves the textual order: forward_proxy runs first, sees CONNECT
// + valid Proxy-Authorization, and tunnels; everything else falls
// through to the static selfsteal under SiteRoot.
//
// IMPORTANT — host matcher. The proxy site address is bare
// `:ListenPort` followed by `Domain:ListenPort`, NOT just
// `Domain:ListenPort`. Reason: a single named address compiles to a
// route guarded by a `host=Domain` matcher, and HTTP/1.1 CONNECT
// requests carry `Host: <target>:port` (e.g. example.com:443) — they
// never match the proxy's own hostname and so are dropped at routing,
// 200ed by Caddy's empty default response, never reaching
// forward_proxy. NaïveProxy clients themselves use HTTP/2 where
// `:authority` equals the proxy's own hostname so they would match,
// but operators pointing curl / pacparser / generic HTTP/1.1 clients
// at the proxy must work too. The bare `:ListenPort` address creates
// a sibling matcher that accepts any Host on that port, while the
// named `Domain:ListenPort` address still drives Caddy's automatic
// TLS to issue an LE certificate for Domain.
